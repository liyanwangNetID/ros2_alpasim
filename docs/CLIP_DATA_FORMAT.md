...
```text
stamp_ns = sec multiplied by 1,000,000,000 plus nanosec
```

ROS message payloads preserve their original `sec` and `nanosec` structure. Wall-clock metadata timestamps are diagnostic only and must not be treated as simulation time.
...
`metadata.json` records the Clip number, name, format, status, camera names, recorder settings, and lifecycle wall times.
...
- Topic counts
- calibration status
- first and last simulation time
- simulation duration
...
`has_complete_gt_service` is recorded but is not part of the recorder rename gate. Downstream consumers that require complete-recording GT must check it explicitly.
...
Each camera uses an independent bounded writer queue. Queue drops or encoding failures invalidate the Clip under the current recorder rules.
...
Simulator Actor truth includes Actors that may not be camera-observable. Step 7 must apply an observability gate before using Actor truth in final supervision.

### Actor history and future

`actors/history.jsonl` is optional for the current Step 7 v0.1 design because a short past window can be derived from `actors/current.jsonl` snapshots at or before the Anchor.
...
`map/vector_map.json` stores frame, map and scene IDs, bounds, lanes, road edges, traffic signs, and wait lines. It is required by recorder validation and used by Steps 4, 6, and planned Step 7 road-context generation.
...
Uses Candidate Anchors, Meta-actions, and Step 4 features to select Keyframes. It produces `manifests/keyframe_contract_v0.1.json` linking the selected Keyframe file to its count and SHA-256.

### Step 6

Uses only Anchor-time-or-earlier Route, Anchor-time Ego state, and static VectorMap. It validates the Keyframe contract and exact Anchor coverage.

### Step 7 current implementation

Step 7 currently uses:

- selected Keyframes
- exact or bounded current Ego and Actor snapshots
- current and past snapshots at or before the Anchor
- four camera calibrations
- F-theta projection
...
- static VectorMap for future road-context work
...
The active geometric occlusion v02 exporter validates `keyframe_contract_v0.1.json`. Its formal Actor-row product covers 3,474 of 3,500 Keyframes; the remaining 26 Keyframes each have one exact current Actor snapshot with an empty Actor list.
...
- cache large files where appropriate
...
3. update Step 2 reader compatibility;
4. update Step 1 Manifest validation;
5. add synthetic and real-data tests;
6. state whether old Clips remain supported.

Changes to annotation code or the artifact output root alone do not change the raw Clip format.
...
Step 7 raw-data consumers and geometry now live under:
