"""Step 7G current road, lane, and wait-line features with exact spatial indexing."""
...
from step2.vector_map_reader import (
    NearbyLane,
    VectorMapReader,
    point_in_polygon,
...
class RoadFeatureMapContext:
    ...
    def __init__(
        ...
        *,
        ...
        maximum_query_radius_m: float = LANE_SEARCH_RADIUS_M,
        cell_size_m: float = SPATIAL_CELL_SIZE_M,
    ) -> None:
...
def match_point_to_road(
    *,
    context: RoadFeatureMapContext,
    point: Point2D,
    yaw_rad: float | None = None,
    maximum_heading_error_rad: float | None = None,
    radius_m: float = LANE_SEARCH_RADIUS_M,
) -> RoadLaneMatch:
    candidates = context.find_nearby_lanes(
        point,
        radius_m=radius_m,
        yaw_rad=yaw_rad,
    ...
    if not candidates:
        return RoadLaneMatch(
            "unmatched", None, None, None, None, None, None, None, (),
    ...
    available = tuple(
        wait_id
        for wait_id in lane.wait_line_ids
        if wait_id in context.wait_lines_by_id
    )
    nearest = None
    ...
    return RoadLaneMatch(
        ...
        lane.lane_id,
        candidate.distance_m,
        candidate.projection.arc_length_m,
        lane.length_m,
        candidate.inside_polygon,
        candidate.heading_error_rad,
        bool(available),
        available,
        None if nearest_item is None else str(nearest_item["id"]),
        None if nearest_item is None else str(nearest_item.get("wait_line_type", "UNKNOWN")),
        None if nearest is None else nearest[0][0],
        None if nearest_item is None else bool(nearest_item.get("is_implicit", False)),
        bool(evidence),
        tuple(evidence),
    )


def compute_ego_and_actor_road_features(
    *,
    context: RoadFeatureMapContext,
    ego_pose: Pose2D,
    actors: Sequence[Mapping[str, Any]],
) -> tuple[RoadLaneMatch, tuple[tuple[str, str, RoadLaneMatch, str], ...]]:
    ego = match_point_to_road(
        context=context,
        point=Point2D(ego_pose.x, ego_pose.y),
        yaw_rad=ego_pose.yaw,
        maximum_heading_error_rad=EGO_MAX_HEADING_ERROR_RAD,
    )
    rows = []
    seen = set()
    for actor in actors:
        ...
        match = match_point_to_road(
            context=context,
            point=Point2D(pose.x, pose.y),
        )
        relation = (
            "unknown"
            if ego.lane_id is None or match.lane_id is None
            else context.vector_map.relation(ego.lane_id, match.lane_id)
        )
