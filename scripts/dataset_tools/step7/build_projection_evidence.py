...
def point_list(points) -> list[dict[str, float]]:
    return [{'u': float(point.u), 'v': float(point.v)} for point in points]

def bbox_dict(bbox) -> dict[str, float]:
    return {'min_u': float(bbox.min_u), 'min_v': float(bbox.min_v), 'max_u': float(bbox.max_u), 'max_v': float(bbox.max_v)}

def evidence_record(*, keyframe: dict[str, Any], camera_name: str, actor: dict[str, Any], projection) -> dict[str, Any]:
    ...
    record = {'schema_version': SCHEMA_VERSION, 'anchor_id': str(keyframe['anchor_id']), 'clip_id': str(keyframe['clip_id']), 'anchor_ns': int(keyframe['anchor_ns']), 'camera_name': camera_name, 'track_id': str(actor['track_id']), 'label_class': str(actor['label_class']), 'is_static': bool(actor['is_static']), 'projection_valid': True, 'sampling_mode': 'adaptive', 'camera_sample_count': int(projection.camera_sample_count), 'projected_bbox': bbox_dict(projection.projected_bbox), 'clipped_bbox': bbox_dict(projection.clipped_bbox), 'projected_hull': point_list(projection.projected_hull), 'clipped_hull': point_list(projection.clipped_hull), 'projected_area_px': float(projection.projected_area_px), 'inside_image_area_px': float(projection.inside_image_area_px), 'inside_image_ratio': float(projection.inside_image_ratio), 'projected_hull_area_px': float(projection.projected_hull_area_px), 'inside_image_hull_area_px': float(projection.inside_image_hull_area_px), 'inside_image_hull_ratio': float(projection.inside_image_hull_ratio), 'projected_height_px': float(projection.projected_height_px), 'minimum_depth_m': float(projection.minimum_depth_m), 'maximum_depth_m': float(projection.maximum_depth_m), 'truncated': bool(projection.truncated)}
    numeric_fields = ('projected_area_px', 'inside_image_area_px', 'inside_image_ratio', 'projected_hull_area_px', 'inside_image_hull_area_px', 'inside_image_hull_ratio', 'projected_height_px', 'minimum_depth_m', 'maximum_depth_m')
    ...
    if not 0.0 < record['inside_image_hull_ratio'] <= 1.0 + 1e-09:
...
def main() -> int:
            ...
            try:
                if clip_id != current_clip_id:
                    ...
                    for camera_name in CAMERA_NAMES:
                        ...
                        frame = exact.value
                        calibrations[camera_name] = load_camera_calibration(reader.clip_directory / 'calibration' / f'{camera_name}.json', camera_name=camera_name, source_width=frame.width, source_height=frame.height)
                ...
                for camera_name in CAMERA_NAMES:
                    ...
                    for actor in actors:
                        pair_count += 1
                        projection = project_actor_box_to_camera(actor, recorded_ego_message=ego.message, calibration=calibration)
                        if projection.edge_samples_per_edge != 0:
                            raise RuntimeError('Production projection did not use adaptive sampling')
                        if not projection.projection_valid:
                            invalid_reasons[projection.failure_reason or 'missing_failure_reason'] += 1
                            continue
                        record = evidence_record(keyframe=keyframe, camera_name=camera_name, actor=actor, projection=projection)
                        unique_key = (anchor_id, camera_name, str(actor['track_id']))
                        ...
                        rows_by_camera_class[f"{camera_name}|{actor['label_class']}"] += 1
                        truncated_count += int(projection.truncated)
            except Exception as exc:
                processing_failures.append({'anchor_id': anchor_id, 'clip_id': clip_id, 'error_type': type(exc).__name__, 'error': str(exc)})
            if index % 100 == 0 or index == len(keyframes):
                print(f'Progress: {index}/{len(keyframes)} pairs={pair_count} rows={valid_count} elapsed_sec={time.monotonic() - started:.1f}', flush=True)
    ...
    summary = {'schema_version': SCHEMA_VERSION, 'description': 'Valid Step 7D geometric projection evidence only. No observability classification or Actor-to-Actor occlusion.', 'source': {'keyframes_path': str(KEYFRAME_PATH), 'keyframe_count': len(keyframes), 'actor_count': actor_count, 'actor_camera_pair_count': pair_count}, 'output_path': str(OUTPUT_PATH), 'valid_evidence_row_count': valid_count, 'invalid_projection_count': sum(invalid_reasons.values()), 'invalid_projection_reasons': dict(sorted(invalid_reasons.items())), 'truncated_valid_row_count': truncated_count, 'rows_by_camera': dict(sorted(rows_by_camera.items())), 'rows_by_actor_class': dict(sorted(rows_by_class.items())), 'rows_by_camera_and_actor_class': dict(sorted(rows_by_camera_class.items())), 'projection_policy': {'sampling': 'adaptive', 'maximum_chord_error_px': 1.0, 'maximum_adaptive_depth': 14, 'near_plane_m': 0.001}, 'processing_failure_count': 0, 'duplicate_key_count': 0, 'elapsed_seconds': time.monotonic() - started}
