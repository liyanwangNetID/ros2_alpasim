from step7.actor_role_selection_v01 import (
    ActorRoleCandidate,
    empty_role_reasons,
    select_actor_roles,
)
...
def test_selects_three_distinct_roles_with_topology_priority():
    roles = select_actor_roles(candidates=(
        candidate("lead", 10, 0, "same"),
        candidate("left", 2, 3, "left_adjacent"),
        candidate("right", -2, -3, "right_adjacent"),
    ))
...
def test_invisible_and_nonvehicle_candidates_are_excluded():
    roles = select_actor_roles(candidates=(
        candidate("hidden", 5, 0, "same", "shadow_not_visible"),
        candidate("person", 5, 0, "same", label="person"),
    ))
...
def test_history_quality_is_not_a_hard_gate():
    ...
    value = ActorRoleCandidate(
        value.track_id, value.label_class, value.geometry, value.visibility,
        {"history_status": "insufficient_span", "mean_distance_rate_mps": None},
        value.road,
    )
...
def test_side_roles_reject_far_lateral_and_far_longitudinal_fallbacks():
    roles = select_actor_roles(candidates=(
        candidate("far_lateral", 2, 19, "unknown"),
        candidate("far_forward", 42, -3, "unrelated"),
    ))
...
def test_side_role_boundary_accepts_nearby_geometric_fallback():
    roles = select_actor_roles(candidates=(
        candidate("left", -10, 7.5, "unknown"),
        candidate("right", 25, -6, "unrelated"),
    ))
    assert roles["left_nearby_vehicle"]["track_id"] == "left"
    assert roles["right_nearby_vehicle"]["track_id"] == "right"
