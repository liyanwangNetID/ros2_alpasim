...
def main() -> int:
    ...
    if (
        args.review_script.expanduser().resolve()
        == DEFAULT_REVIEW_SCRIPT.resolve()
    ):
        command = [
            sys.executable,
            "-m",
            "step4.review_lateral_action",
        ]
        command_directory = TOOL_ROOT.parent
    else:
        command = [
            sys.executable,
            str(args.review_script),
        ]
        command_directory = args.review_script.parent
